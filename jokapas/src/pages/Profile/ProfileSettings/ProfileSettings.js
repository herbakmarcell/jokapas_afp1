...
export function ProfileSettings({showActiveTab}) {
    ...
    return (
        <ul className="profile-setting-ul">
            ...
            <li className={active === 3 ? "profile-setting-li active" : "profile-setting-li"} onClick={()=> updateActiveTab(3)}>2FA</li>
            <li className={active === 4 ? "profile-setting-li active" : "profile-setting-li"} onClick={()=> updateActiveTab(4)}>Felhasználó törlése</li>
        </ul>
    )
}